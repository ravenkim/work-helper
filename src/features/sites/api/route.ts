...
export async function GET(request: Request) {
    ...
    if (searchTerm) {
        filteredSites = filteredSites.filter(site =>
            ...
            site.labels.some(label => label.toLowerCase().includes(searchTerm))
        );
    }
        ...
        paginatedSites.map(async (site) => {
            try {
                ...
                return {
                    ...
                    labels: Array.from(new Set([...metadata.keywords, ...site.labels])),
                };
    ...
    const nextCursor = startIndex + ITEMS_PER_PAGE < filteredSites.length
