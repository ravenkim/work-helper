...
const getErrorMessage = (
    ...
    return messages[status] || (responseData as { message?: string })?.message || fallbackMessage
}
...
>(
    ...
    const asyncReducers = asyncRequests.reduce(
        (reducers, { action, state: stateKey }) => ({
            ...
            [`${action}Success`]: (state: typeof asyncState, action: { payload: unknown }) => ({
                ...state,
                [stateKey]: reducerUtils.success(action.payload),
            }),
            [`${action}Fail`]: (state: typeof asyncState, action: { payload: string }) => ({
                ...state,
                [stateKey]: reducerUtils.error(state[stateKey]?.data, action.payload),
            }),
        }),
        {},
    )
